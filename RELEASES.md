...
ambient（本地 session）用量首跑回填最近 7 天历史（TEA-618 W2）。

### 回填，不再只向前
此前 ambient 采集器只向前：首跑把水位种在当前末尾、不补历史，成员 daemon 升级后 Users 页要等产生新用量才有数。本版两个采集器（Claude / Codex）在首跑（全新 daemon，或已 seed 的老 daemon 升级到本版）各跑一次窗口回填——把最近 `MULTICA_AMBIENT_BACKFILL_DAYS`（默认 7）天的用量按**真实发生时间**补进库（服务端按事件时间分桶，不灌当前小时）。`<=0` 关闭回填、退回只向前。

### Codex 计数改为逐快照
Codex 采集器重写为逐 token_count 事件 emit 增量（此前每会话每次扫描只发一条、压在末个时间戳）：每个增量落在自己的发生时间，跨回填窗口边界的首条增量 = 窗内首个累计 − 窗前最后累计，Σ 精确等于该会话真实增长；顺带修掉「种子后新建会话把全量累计压在单一时间戳」。

### 大回填分批
回填量大时分批上传（1000/批），任一批失败则水位不前移、下次整体重发，服务端唯一键去重——不丢不重，水位提交保持 all-or-nothing。

### 升级
- 队员 daemon 升级到本版后会**各自回填一次**最近 7 天历史；`multica update`、重跑 `scripts/install.sh`，或设 `MULTICA_DAEMON_AUTO_UPDATE=true` 自更。
