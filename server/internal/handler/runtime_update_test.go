...
func TestInMemoryUpdateStore_HasPending(t *testing.T) {
	...
	if _, err := store.Create(ctx, "rt-1", "v1.2.3", false); err != nil {
...
func TestInMemoryUpdateStore_PopPendingIgnoresTerminalHistory(t *testing.T) {
	...
	first, err := store.Create(ctx, "rt-1", "v1.2.3", false)
	if err != nil {
	...
	second, err := store.Create(ctx, "rt-1", "v1.2.4", false)
	if err != nil {
...
func TestInMemoryUpdateStore_RunningRequestTimesOut(t *testing.T) {
	...
	req, err := store.Create(ctx, "rt-timeout", "v1.2.3", false)
	if err != nil {
		t.Fatalf("create: %v", err)
	}
...
func TestInMemoryUpdateStore_RejectsConcurrentActiveUntilTerminal(t *testing.T) {
	...
	req, err := store.Create(ctx, "rt-1", "v1.2.3", false)
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := store.Create(ctx, "rt-1", "v1.2.4", false); err != errUpdateInProgress {
		t.Fatalf("second create error = %v, want errUpdateInProgress", err)
	}
	if err := store.Complete(ctx, req.ID, "done"); err != nil {
		t.Fatalf("complete: %v", err)
	}
	if _, err := store.Create(ctx, "rt-1", "v1.2.4", false); err != nil {
