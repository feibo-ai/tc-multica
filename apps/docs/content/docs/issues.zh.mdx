...
Issue 可以按标签筛选，并明确选择 AND / OR 语义。在工作区的 Issues 视图里打开 **标签（Labels）** 筛选器，用 **匹配（Match）** 开关切换：

- **任一**（默认）：展示带有*任一*所选标签的 issue（OR）。
- **全部**：只展示*同时*带有所有所选标签的 issue（AND）。

CLI 行为一致。每个标签传一次 `--label`，再加 `--labels-mode any|all`：

```bash
multica issue list --label plan-approved --label urgent --labels-mode all
```

对应的 API 接口是 `GET /api/issues?labels=plan-approved,urgent&labels_mode=all`——基于标签名、忽略大小写。如果想用 UUID，可以继续使用现有的 `?label_ids=` 形式。当前阶段标签筛选请单独使用；在服务端将它与 `status` / `priority` / `assignee_id` 组合使用是后续规划。
