...
对应的 API 接口是 `GET /api/issues?labels=待审批,紧急&labels_mode=all`——基于标签名、忽略大小写。如果想用 UUID，可以继续使用现有的 `?label_ids=` 形式。当前阶段标签筛选请单独使用；在服务端将它与 `status` / `priority` / `assignee_id` 组合使用是后续规划。
