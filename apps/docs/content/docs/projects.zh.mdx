...
每个项目都带一个明确的 `dri_user_id` —— 对结果负总责的那个**人**，和 `lead_type` / `lead_id`（可以是智能体或 squad，负责推动执行）是两回事。DRI 必须是工作区成员；lead 可以是真正在干活的智能体。把两者分开，是为了让你能把执行交给智能体，同时还有一个明确的人在结果这一头担着。

CLI 用 `multica project assign-dri <project> <user-uuid>` 来设置，`project create` / `project update` 上的 `--dri` 参数也可以一并设。`multica project list --without-dri` 和 web UI 的 **Without DRI** 筛选 chip 会把处于 SOP P-5 风险的项目列出来 —— 也就是没有人挂名的工作。项目详情页上的 **DRI** 选择器随时可以改。
