...
<Callout type="warn">
**边界声明：`apps/docs/` 是上游产品文档快照。** 本团队自部署的 Multica CLI 以仓库顶层 [`CLI_INSTALL.md`](https://github.com/feibo-ai/tc-multica/blob/main/CLI_INSTALL.md) / [`CLI_AND_DAEMON.md`](https://github.com/feibo-ai/tc-multica/blob/main/CLI_AND_DAEMON.md) 为准（发版于 `feibo-ai/tc-multica` Releases）。本文档站内若仍出现 Homebrew (`multica-ai/tap`) 或 `multica-ai/multica` 的安装指引，均指上游公网产品，不适用本部署——安装/升级请改用 `scripts/install.sh` 与 `multica update`。
</Callout>
