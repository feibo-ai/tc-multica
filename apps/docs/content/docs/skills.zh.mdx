...
每个 Skill 都带一个明确的 `owner_user_id`——当前对这个 Skill 负责的那个人，和 `created_by`（记录最初是谁写的）是两回事。负责人可以随时调整：在 Skill 详情页的负责人选择器里改，或者用 CLI `multica skill update <id> --owner <user-uuid>`。

每个 Skill 还有一个 `last_reviewed_at` 时间戳。CLI 的 `multica skill list --stale` 和工作区 UI 的 **Stale**（过期）筛选会把超过 90 天未复核的 Skill 列出来——这些就是 SOP 反模式 ❌5 的候选（陈旧知识在工作区里悄悄烂掉）。在 Skill 详情页点 **Mark reviewed**（标记已复核）完成签字后重置计时；这个动作会把 `last_reviewed_at` 盖戳为 `now()`。
