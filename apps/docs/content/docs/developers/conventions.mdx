...
Always live under `/{slug}/{section}` — `/{slug}/projects`, `/{slug}/agents`, `/{slug}/settings`. Never duplicate workspace routing logic; use `useNavigation().push()` from shared code, never framework-specific link APIs. (`/{slug}/issues` is the retired Issues-list route: still a reserved slug, it now redirects to the unified `/{slug}/projects` tab; individual issues live at `/{slug}/issues/:id`.)
