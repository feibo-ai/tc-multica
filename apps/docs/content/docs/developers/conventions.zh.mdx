...
永远用 `/{slug}/{section}` —— `/{slug}/projects`、`/{slug}/agents`、`/{slug}/settings`。共享代码不要复制路由逻辑，统一走 `useNavigation().push()`，不要直接用框架的 link API。(`/{slug}/issues` 是已退役的 Issues 列表路由:仍是保留 slug,现在重定向到统一的 `/{slug}/projects` 标签页;单个 issue 仍在 `/{slug}/issues/:id`。)
