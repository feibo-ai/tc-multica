...
```bash
multica issue list --label 待审批 --label 紧急 --labels-mode all
```

The API endpoint is `GET /api/issues?labels=待审批,紧急&labels_mode=all` — name-based and case-insensitive. If you prefer UUIDs, use the existing `?label_ids=` shape. Use the labels filter on its own for now; composing it with `status`, `priority`, or `assignee_id` server-side is a planned follow-up.
