...
脚本会从 GitHub Releases 下载最新 CLI 二进制并加入 `PATH`。重跑即可升级，或运行 `multica update`。

> **不要用 Homebrew**（`brew install multica-ai/tap/multica`）：那个 tap 属于上游开源项目、不会更新本部署的发布版本，装上的是另一套陈旧版本。
